...
async function loadFormats() {
  try {
    const resp = await fetch("formats.json");
...
async function copyBibtex() {
  const text = document.getElementById("bibtexOutput").innerText.trim();
  if (!text) {
    showMessage("No BibTeX to copy.", "error");
    return;
  }
  try {
    await navigator.clipboard.writeText(text);
    showMessage("BibTeX copied to clipboard!", "success");
    setTimeout(hideMessage, 2000);
  } catch (err) {
    console.error(err);
    showMessage("Error copying BibTeX.", "error");
  }
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("copyCitationButton")
    .addEventListener("click", copyCitation);
  document
    .getElementById("copyBibtexButton")
    .addEventListener("click", copyBibtex);
